...
final class LoggerPrinter implements Printer {
	...
	@Override
	public void e(Throwable throwable, String message, final Object... args) {
		final Throwable throwableForSave = throwable;
		final String messageForSave = message;
		if (settings.getErrorLogSaveFile() != null) {
			new Thread(new Runnable() {
				@Override
				public void run() {
					FileOperator.getInstance(settings.getErrorLogSaveFile()).saveToFile(throwableForSave, messageForSave, args);
				}
			}).run();
		}
	...
	private synchronized void log(int logType, String msg, Object... args) {
		if (settings.getLogLevel() == LogLevel.NONE) {
			return;
		} else if (settings.getLogLevel() == LogLevel.ASSERT && logType < ASSERT) {
			return;
		} else if (settings.getLogLevel() == LogLevel.ERROR && logType < ERROR) {
			return;
		} else if (settings.getLogLevel() == LogLevel.WARNING && logType < WARN) {
			return;
		} else if (settings.getLogLevel() == LogLevel.INFO && logType < INFO) {
			return;
		} else if (settings.getLogLevel() == LogLevel.DEBUG && logType < DEBUG) {
			return;
		} else if (settings.getLogLevel() == LogLevel.VERBOSE && logType < VERBOSE) {
			return;
		}
		String tag = getTag();
